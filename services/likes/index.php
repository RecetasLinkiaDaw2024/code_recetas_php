<?php
/***
* SERVICIO DE LIKE - DISLIKE
* JSON CON PARAMETROS ID-RECETA Y TIPO Y AÑADIMOS UN LIKE O UN DISLIKE
*/
...
} else if($_SERVER['REQUEST_METHOD'] === 'GET' && isset($_GET['id-receta']) && isset($_GET['tipo'])){
    if ($_GET['tipo'] == 'L'){
        $num_likes = contarLikesReceta($_GET['id-receta']);
    }else{
        $num_likes = contarDisLikesReceta($_GET['id-receta']);
    }
     // Crear un arreglo asociativo con el número de likes
     $response_data = array('num-likes' => $num_likes);
